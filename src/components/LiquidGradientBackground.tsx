    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
            ...
            vec3 getGradientColor(vec2 uv, float time) {
              ...
              if (uGradientCount.value > 6.0) {
                color += uColor1 * influence7 * (0.55 + 0.45 * sin(time * uSpeed * 1.4)) * uColor1Weight;
                color += uColor2 * influence8 * (0.55 + 0.45 * cos(time * uSpeed * 1.5)) * uColor2Weight;
                color += uColor3 * influence9 * (0.55 + 0.45 * sin(time * uSpeed * 1.6)) * uColor1Weight;
                color += uColor4 * influence10 * (0.55 + 0.45 * cos(time * uSpeed * 1.7)) * uColor2Weight;
              }
              if (uGradientCount.value > 10.0) {
              ...
              color = clamp(color, vec3(0.0), vec3(1.0)) * uIntensity.value;
              ...
              color = mix(uDarkNavy.value, color, mixFactor1);
...
// App class for managing the Three.js scene
class App {
...
const LiquidGradientBackground: React.FC = () => {
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      // 直接设置renderer dom元素的样式
      const canvas = appRef.current.renderer.domElement;
      if (canvas) {
